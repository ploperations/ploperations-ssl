  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      when 'RedHat'
        ...
        it { is_expected.to contain_file('/etc/pki/certs/www.example.com_inter.crt').with_ensure('file') }
        it { is_expected.to contain_file('/etc/pki/certs/www.example.com_combined.crt').with_ensure('file') }
      else
        ...
        it { is_expected.to contain_file('/etc/ssl/certs/www.example.com_inter.crt').with_ensure('file') }
        it { is_expected.to contain_file('/etc/ssl/certs/www.example.com_combined.crt').with_ensure('file') }
      end
    end
  end
end
